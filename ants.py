        ...
        if insect.is_ant:
            if isinstance(insect, QueenAnt):
                if insect.true_queen:
                    ...
                    self.ant = None
            elif insect.container and isinstance(insect.ant, QueenAnt):
                insect = insect.ant
                if insect.true_queen:
                    self.ant = insect
                    return
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        if self.container and not other.container and self.ant is None:
            return True
        return False
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        place = self.place
        steps = 0
        while place != hive and steps <= self.max_range:
            if self.min_range <= steps and place.bees != []:
    ...
    def throw_at(self, target):
        ...
        if target is not None:
            ...
            Ant.reduce_armor(target, self.damage)
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 4A
        if self.armor - amount <= 0: #< add 
            bees_copy = self.place.bees.copy()
            new_bees = []
            for i in range(len(self.place.bees)):
                bees_copy[i].reduce_armor(self.damage)
                if bees_copy[i].armor != 0:
                    new_bees.append(bees_copy[i])
            self.place.bees = new_bees

        Ant.reduce_armor(self, amount) #< moved this down here
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.ants_doubled_behind = []
        
        if QueenAnt.spawned == False:
            QueenAnt.spawned = True
    ...
    def action(self, colony):
        ...
        if self.true_queen:
            ...
            while p != None:
                if p.ant != None:
                    if p.ant not in self.ants_doubled_behind:
                        if not isinstance(p.ant, QueenAnt):
                            p.ant.damage = p.ant.damage*2
                            self.ants_doubled_behind.append(p.ant)
                    if p.ant.container and p.ant.ant != None and p.ant.ant not in self.ants_doubled_behind:
                        if not isinstance(p.ant.ant, QueenAnt):                    
                            p.ant.ant.damage = p.ant.ant.damage*2
                            self.ants_doubled_behind.append(p.ant.ant)
                p = p.exit
...
def apply_effect(effect, bee, duration):
    ...
    if effect is make_slow:
        for s in range(duration):
            make_slow(bee.action)

    elif effect is make_stun:
        for s in range(duration):
            make_stun(bee.action)
